...
class Fusion_Builder_Conditional_Render_Helper {
	...
	public static function get_value( $name, $value, $additionals ) {
		...
		switch ( $name ) {
			...
			case 'get_var':
				if ( ! isset( $_GET[ $additionals ] ) ) {
					return null;
				}
				if ( is_array( $_GET[ $additionals ] ) ) {
					return array_map( 'sanitize_text_field', $_GET[ $additionals ] );
				}

				return sanitize_text_field( wp_unslash( $_GET[ $additionals ] ) );
			case 'custom_field':
