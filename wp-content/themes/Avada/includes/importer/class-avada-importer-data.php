...
class Avada_Importer_Data {
	...
	/**
	 * Action used to set data transient.
	 * 
	 * @since 7.10
	 * @return void
	 */
	public static function set_data_transient() {
		self::get_data();
	}
	...
	private function xml_replacements() {
		...
		$base = false === strpos( $xml_content, 'avada.website' ) ? 'https://avada.theme-fusion.com/' : 'https://avada.website/';
		...
		$xml_content = str_replace(
			[
				...
				'http://avada.website/' . $demo,
				'https://avada.website/' . $demo,
			],
			$home_url,
			$xml_content
		);
		...
		$json_content = str_replace(
			[
				...
				str_replace( '/', '\\/', 'http://avada.website/' . $demo ),
				str_replace( '/', '\\/', 'https://avada.website/' . $demo ),
			],
		...
		$xml_content = str_replace(
			$home_url . '/wp-content/',
			$base . $demo . '/wp-content/',
			$xml_content
		);

		// Take care of assets.
		if ( 'https://avada.theme-fusion.com/' === $base ) {
			...
			$xml_content = preg_replace_callback( '/(?<=<wp:meta_value><!\[CDATA\[).*(https?:\/\/avada.website)+(.*?)(?=]]><)/', 'fusion_fs_importer_replace_url', $xml_content );
		}

		// Replace URLs in the JSON file.
		$json_content = str_replace(
			str_replace( '/', '\\/', $home_url . '/wp-content/' ),
			str_replace( '/', '\\/', $base . $demo . '/wp-content/' ),
			$json_content
		);

		// Widget replacements.
		$site_url     = str_replace( '/', '\\/', str_replace( 'https:', '', $base ) . $demo . '/wp-content/uploads/sites/' );
		...
		if ( false !== $widget_image ) {
			...
			$uploads_url = [
				...
				str_replace( '/', '\\/', 'http://avada.website/' . $demo . '/wp-content/uploads/sites/' . $site_id ),
				str_replace( '/', '\\/', 'https://avada.website/' . $demo . '/wp-content/uploads/sites/' . $site_id ),
			];
